...
namespace
{
    ...
    const unsigned char* Glyph(char c)
    {
        ...
        switch (c) { case '-': return dash; case '.': return dot;
        case ':': return colon; case '/': return slash; case '+': return plus;
        case '[': return leftBracket; case ']': return rightBracket; }
        return space;
    }
}
...
{
    // Embedded for this geometry prototype: launching the EXE needs no shader cwd.
    const char* vs = "#version 330 core\n"
        "layout(location=0) in vec2 position; layout(location=1) in vec4 color;"
        "layout(location=2) in vec2 texcoord; out vec2 uv;"
        "layout(location=3) in float energy; out float radiance;"
        "uniform vec2 viewport; out vec4 tint; void main(){"
        "gl_Position=vec4(position.x/viewport.x*2.0-1.0,"
        "1.0-position.y/viewport.y*2.0,0.0,1.0); tint=color; uv=texcoord; radiance=energy;}";
    const char* fs = "#version 330 core\n"
        "in vec4 tint; in vec2 uv; uniform sampler2D textImage; uniform bool textured;"
        "in float radiance; uniform bool linearOutput; out vec4 fragment;"
        "vec3 decode(vec3 c){return mix(pow(max((c+0.055)/1.055,vec3(0.0)),vec3(2.4)),"
        "c/12.92,lessThanEqual(c,vec3(0.04045)));}"
        "void main(){vec3 rgb=linearOutput ? decode(tint.rgb)*radiance : tint.rgb;"
        "fragment=vec4(rgb,"
        "tint.a*(textured ? texture(textImage,uv).r : 1.0));}";
    ...
    GLint success = GL_FALSE;
    if (program_) glGetProgramiv(program_, GL_LINK_STATUS, &success);
    if (!success)
    {
        char log[2048] = {};
        if (program_) glGetProgramInfoLog(program_, sizeof(log), nullptr, log);
        std::cerr << "Scene program: " << log << '\n';
        Shutdown();
        return false;
    }
    ...
    bool initialized = vao_ && vbo_ && viewport_ >= 0 && linearOutput_ >= 0;
    if (initialized) postProcessor_.Initialize(); // failure falls back to the direct path
    return initialized;
}
...
void SceneRenderer::Shutdown()
{
    ...
    for (const auto& entry : textCache_) glDeleteTextures(1, &entry.second.texture);
    textCache_.clear();
    drawingTexture_ = 0;
...
{
    for (Point2 p : {a, b, c}) vertices_.push_back({p.x,p.y,color.r,color.g,color.b,color.a,0,0,color.energy});
}
...
void SceneRenderer::Line(Point2 a, Point2 b, float width, Color color)
{
    ...
    Quad({a.x+ox,a.y+oy},{b.x+ox,b.y+oy},{b.x-ox,b.y-oy},{a.x-ox,a.y-oy},color);
}
...
const SceneRenderer::TextImage* SceneRenderer::PrepareKoreanText(
    const std::wstring& text, int fontSize, int wrapWidth)
{
    ...
    if (cached != textCache_.end()) return &cached->second;
    ...
    int measured = DrawTextW(dc, text.c_str(), static_cast<int>(text.size()), &bounds, flags | DT_CALCRECT);
    ...
    if (!image.texture) return nullptr;
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, image.texture);
    GLint alignment = 4;
    glGetIntegerv(GL_UNPACK_ALIGNMENT, &alignment);
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R8, bitmapWidth, bitmapHeight, 0, GL_RED, GL_UNSIGNED_BYTE, mask.data());
...
void SceneRenderer::KoreanText(float x, float y, const std::wstring& text, Color color,
    int fontSize, int wrapWidth)
{
    const TextImage* image = PrepareKoreanText(text, fontSize, wrapWidth);
    if (!image) { Text(x,y,"KOREAN FONT ERROR",color,1.5f); return; }
    Flush();
    drawingTexture_ = image->texture;
    float right=x+image->width, bottom=y+image->height;
    for (auto corner : {std::pair<Point2,Point2>{{x,y},{0,0}}, {{right,y},{1,0}},
        {{right,bottom},{1,1}}, {{x,y},{0,0}}, {{right,bottom},{1,1}}, {{x,bottom},{0,1}}})
        vertices_.push_back({corner.first.x,corner.first.y,color.r,color.g,color.b,color.a,
            corner.second.x,corner.second.y});
    Flush();
}
