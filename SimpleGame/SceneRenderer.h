...
struct Color { float r, g, b, a = 1.0f, energy = 1.0f; };
...
public:
    ...
    void FinishWorld();
    PostProcessSettings& Effects() { return postProcessor_.Settings(); }
    bool PostProcessingAvailable() const { return postProcessor_.Available(); }
    void Flush();
    ...
    void KoreanText(float x, float y, const std::wstring& text, Color color,
        int fontSize = 20, int wrapWidth = 600);
private:
    struct Vertex { float x, y, r, g, b, a, u = 0, v = 0, energy = 1.0f; };
